...
const ProductDetails = ({ cartItemCount, setCartItemCount }) => {
  const [product, setProduct] = useState(null);
  const [qty, setQty] = useState(1);
  const { id } = useParams();

  useEffect(() => {
    const apiUrl = `${import.meta.env.VITE_API_URL}/pc-products/${id}`;
    fetch(apiUrl)
      .then((res) => {
        if (!res.ok) {
      ...
      <div className="bg-gray-100 h-full">
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-wrap -mx-4">
            {/* Product Image */}
            <div className="w-auto md:w-1/2 px-4 mb-8">
              <img
              ...
              <div className="mb-6">
                ...
                <div className="inline-flex items-center">
                  ...
                  <input
                    ...
                    value={qty}
                    readOnly
                    className="mx-2 w-16 text-center rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                  />
